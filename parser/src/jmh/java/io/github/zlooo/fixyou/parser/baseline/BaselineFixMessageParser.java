package io.github.zlooo.fixyou.parser.baseline;
...
public class BaselineFixMessageParser implements Resettable {
    ...
    public BaselineFixMessageParser(ByteBufComposer bytesToParse, FixSpec fixSpec, FixMessage fixMessage) {
    ...
    private void setFieldValue(int fieldNumber, FieldType fieldType, int startIndex, int length) {
        switch (fieldType) {
            ...
            case TIMESTAMP:
                ...
                fixMessage.setTimestampValue(fieldNumber, parseTimestamp(bytesToParse, startIndex, length, timestampParser));
                break;
            default:
    ...
    private void setFieldValue(int fieldNumber, int groupNumber, byte repetitionIndex, byte parentGroupRepetitionIndex, FieldType fieldType, int startIndex, int length) {
        switch (fieldType) {
            ...
            case TIMESTAMP:
                timestampParser.reset();
                fixMessage.setTimestampValue(fieldNumber, groupNumber, repetitionIndex, parentGroupRepetitionIndex, parseTimestamp(bytesToParse, startIndex, length, timestampParser));
                break;
            default:
                throw new IllegalArgumentException(UNSUPPORTED_FIELD_TYPE + fieldType);
        }
    ...
    private static long parseTimestamp(ByteBufComposer byteBuf, int srcIndex, int length, DateUtils.TimestampParser timestampParser) {
        final ByteBuf bytes = Unpooled.buffer(length, length);
        byteBuf.getBytes(srcIndex, length, bytes);
        bytes.forEachByte(timestampParser);
        return timestampParser.getResult();
    }
}