...
import pl.zlooo.fixyou.netty.handler.SessionHandler
...
class AcceptorSessionInitializationIntegrationTest extends AbstractFixYOUAcceptorIntegrationTest {
    ...
    def "should disconnect when second logon attempt on same session is done test case 1S-b-2"() {
        ...
        quickfix.SessionID secondSessionID = new quickfix.SessionID("FIXT.1.1", targetCompId, senderCompId, qualifier)
    ...
    def "should disconnect when logon attempt on unknown session is done test case 1S-c-2"() {
        setup:
        quickfix.SessionID sessionIdForUnknownSession = new quickfix.SessionID("FIXT.1.1", senderCompId + "NotConfigured", targetCompId)
