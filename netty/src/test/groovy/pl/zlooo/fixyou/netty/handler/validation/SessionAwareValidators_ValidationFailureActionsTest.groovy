...
class SessionAwareValidators_ValidationFailureActionsTest extends Specification {
    ...
    def "should send reject and logout when original sending time is greater than sending time"() {
        ...
        fixMessage.getField(FixConstants.ORIG_SENDING_TIME_FIELD_NUMBER).value = FixConstants.UTC_TIMESTAMP_FORMATTER.format(now.plusMinutes(1)).toCharArray()
        fixMessage.getField(FixConstants.SENDING_TIME_FIELD_NUMBER).value = FixConstants.UTC_TIMESTAMP_FORMATTER.format(now.minusMinutes(1)).toCharArray()
        ...
        logoutMessage.getField(FixConstants.MESSAGE_TYPE_FIELD_NUMBER).value == FixConstants.LOGOUT
        logoutMessage.getField(FixConstants.TEXT_FIELD_NUMBER).value == LogoutTexts.INACCURATE_SENDING_TIME
        0 * _
    }
    ...
    def "should send reject and logout when sender or target comp id is wrong"() {
        ...
        1*channelFuture2.addListener(FixChannelListeners.LOGOUT_SENT)>>channelFuture2
        1 * channelFuture2.addListener(ChannelFutureListener.CLOSE)
        logout.getField(FixConstants.MESSAGE_TYPE_FIELD_NUMBER).value == FixConstants.LOGOUT
        logout.getField(FixConstants.TEXT_FIELD_NUMBER).value == LogoutTexts.INCORRECT_SENDER_OR_TARGET_COMP_ID
        0 * _
    ...
    def "should send logout when begin string is wrong"() {
        ...
        fixMessage.getField(FixConstants.TEXT_FIELD_NUMBER).value == LogoutTexts.INCORRECT_BEGIN_STRING
        0 * _
    }
    ...
    def "should do nothing if body length is incorrect"() {
        ...
        fixMessage.getField(FixConstants.TARGET_COMP_ID_FIELD_NUMBER).value = sessionID.senderCompID
        fixMessage.getField(FixConstants.SENDER_COMP_ID_FIELD_NUMBER).value = sessionID.targetCompID
        fixMessage.getField(FixConstants.BODY_LENGTH_FIELD_NUMBER).value = 1
