...
import pl.zlooo.fixyou.netty.handler.MutableIdleStateHandler
...
class PipelineUtilsTest extends Specification {
    ...
    def "should not add handlers which are on exclude list"() {
        ...
        /**
         * should be same as in {@link pl.zlooo.fixyou.netty.handler.FIXYouChannelInitializer#initChannel}
         */
    ...
    def "should not add handler if dependant handler is not present"() {
        ...
        Assertions.
                assertThat(channelPipeline.names()).
                containsExactly(Handlers.FLUSH_CONSOLIDATION_HANDLER.getName(), Handlers.MESSAGE_ENCODER.getName(), Handlers.BEFORE_SESSION_MESSAGE_VALIDATOR.getName(), Handlers.GENERIC.getName(), Handlers.SESSION.getName(),
